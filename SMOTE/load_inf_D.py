...
class Dataset:
    def __init__(self, tar_fname, opt_fname, sal_fname, label_fname):
        ...
        self.frame_ids, self.train_lbl, self.gaze_positions, self.durations = self.read_label_file(label_fname)
    ...
    def read_label_file(self, label_fname):
        ...
        with open(label_fname, 'r') as f:
            for line in f:
                ...
                try:
                    frame_id, duration, lbl = dataline[0], int(dataline[3]), dataline[5]
                ...
                binned_durations.append(self.bin_duration(duration))
        
        # Count the number of items in each bin
        bin_counts = np.bincount(binned_durations)
        print(f"Number of items in bin 0 (0-60ms): {bin_counts[0]}")
        print(f"Number of items in bin 1 (>60ms): {bin_counts[1]}")
        
        return np.array(frame_ids), np.array(lbls, dtype=np.int32), gaze_positions, np.array(durations, dtype=np.int32)

    def bin_duration(self, duration):
        bins = [0, 60, float('inf')]  # This creates two bins: 0-60ms and >60ms
        return np.digitize(duration, bins) - 1  # Subtract 1 to make bins 0-indexed
    ...
    def generator(self):
        ...
        for i, frame_id in enumerate(self.frame_ids):
            ...
            if len(imgs_deque) == 4:
                ...
                duration_bin = self.bin_duration(self.durations[i])
                
                self.frame_order.append(frame_id)  # Add frame_id to frame_order list
                
                yield (stacked_img, stacked_opt, stacked_sal), (gaze_map, duration_bin), frame_id
    ...
    def get_dataset(self):
        dataset = tf.data.Dataset.from_generator(
            ...
            output_signature=(
                ...
                tf.TensorSpec(shape=(), dtype=tf.string)  # frame_id
            )
        )

...
class TemporalSMOTEDataset(Dataset):
    ...
    def interpolate_images(self, prev_img, next_img):
        # Ensure images are in the correct format (uint8)
        ...
        # Create map for image warping
        h, w = flow.shape[:2]
        map_x, map_y = np.meshgrid(np.arange(w), np.arange(h))
        map_x = map_x.astype(np.float32) + 0.5 * flow[..., 0]
        map_y = map_y.astype(np.float32) + 0.5 * flow[..., 1]
        
        # Warp prev_img and next_img
        warped_prev = cv2.remap(prev_img, map_x, map_y, cv2.INTER_LINEAR)
        warped_next = cv2.remap(next_img, map_x - flow[..., 0], map_y - flow[..., 1], cv2.INTER_LINEAR)
        
        # Blend warped images and normalize back to [0, 1] range
        return (0.5 * warped_prev + 0.5 * warped_next).astype(np.float32) / 255.0

    def interpolate_gaze_maps(self, prev_gaze_map, next_gaze_map):
        # Ensure gaze maps are in the correct format (uint8)
        prev_gaze_map = (prev_gaze_map * 255).astype(np.uint8)
        next_gaze_map = (next_gaze_map * 255).astype(np.uint8)
        
        # Calculate optical flow
        flow = cv2.calcOpticalFlowFarneback(prev_gaze_map, next_gaze_map, None, 0.5, 3, 15, 3, 5, 1.2, 0)
        ...
        warped_prev = cv2.remap(prev_gaze_map, map_x, map_y, cv2.INTER_LINEAR)
        warped_next = cv2.remap(next_gaze_map, map_x - flow[..., 0], map_y - flow[..., 1], cv2.INTER_LINEAR)
        
        # Blend warped gaze maps and normalize back to [0, 1] range
        return (0.5 * warped_prev + 0.5 * warped_next).astype(np.float32) / 255.0
    ...
    def generator(self):
        ...
        for sample in self.synthetic_samples:
            ...
            # Interpolate images using optical flow
            ...
            # Interpolate gaze map using optical flow

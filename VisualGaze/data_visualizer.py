...
def visualize_csv(tar_fname, csv_fname, predictions_dir):
    ...
    def create_heatmap_surface(heatmap, size):
        # Resize heatmap to match the screen size
        ...
        # Apply the heatmap colors
        for x in range(size[0]):
            for y in range(size[1]):
                value = heatmap.get_at((x, y))[0]  # Get red channel as intensity
                heatmap_surface.set_at((x, y), (0, 255, 0, int(value * 0.3)))  # Green with 30% max opacity
    ...
    while True: #ds.cur_frame_id < ds.total_frame:
        ...
        if not os.path.isfile(png_fname):
            ...
            # Draw predicted gaze heatmap
            if frame_id in predictions:
                pred_heatmap = predictions[frame_id]
                heatmap_surface = create_heatmap_surface(pred_heatmap, (w, h))
        ...
        if duration is not None:
            time.sleep(duration * 0.00001)  # duration is in msec
...
if __name__ == '__main__':
    ...
    # tar_fname = "./ms_pacman/52_RZ_2394668_Aug-10-14-52-42.tar.bz2"
    # csv_fname = "./ms_pacman/52_RZ_2394668_Aug-10-14-52-42.txt"
    
    tar_fname = "./ms_pacman/496_RZ_3560871_Jul-19-13-28-35.tar.bz2"
    csv_fname = "./ms_pacman/496_RZ_3560871_Jul-19-13-28-35.txt"
    predictions_dir = "./ms_pacman/predictions"  # Directory containing predicted gaze heatmaps
    
    visualize_csv(tar_fname, csv_fname, predictions_dir)

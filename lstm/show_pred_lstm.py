...
def match_and_rename_predictions(npz_file, input_dir, output_dir):
    ...
    for i, frame_id in enumerate(frame_ids):
        ...
        if duration_bin == 0:
            duration_value = f"0-{bins[0]}"
        elif duration_bin == len(bins):
            duration_value = f"{bins[-1]}+"
        else:
...
# Usage
npz_file = "./ms_pacman/gaze_lstm.npz"
input_dir = "./ms_pacman/test"  # Directory containing original input images
output_dir = "./ms_pacman/predictions_lstm"  # Directory to save renamed prediction images
